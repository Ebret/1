...
// Function to calculate sponsor commissions
function calculateSponsorCommissions($member_id, $sale_amount, $pdo) {
    try {
        $commissions = [];

        // Get member's sponsor chain
        $stmt = $pdo->prepare("SELECT upline_path FROM mlm_genealogy WHERE member_id = ?");
        $stmt->execute([$member_id]);
        $genealogy = $stmt->fetch();

        if ($genealogy && $genealogy['upline_path']) {
            $upline_ids = explode(',', $genealogy['upline_path']);

            // Calculate sponsor bonuses (2% for direct sponsor, 1% for level 2, 0.5% for level 3)
            $bonus_rates = [0.02, 0.01, 0.005]; // 2%, 1%, 0.5%

            for ($i = 0; $i < min(count($upline_ids), 3); $i++) {
                $sponsor_id = $upline_ids[count($upline_ids) - 1 - $i]; // Start from direct sponsor
                $bonus_rate = $bonus_rates[$i];
                $bonus_amount = $sale_amount * $bonus_rate;

                // Insert sponsor commission
                ...
                ) VALUES (?, ?, 'indirect', ?, ?, ?, ?, 'pending')");

                $stmt->execute([
                    $sponsor_id, $member_id, $i + 1, $sale_amount,
                    $bonus_rate * 100, $bonus_amount
                ]);
        ...
        error_log("Sponsor commission calculation error: " . $e->getMessage());
        return [];
    }
}
